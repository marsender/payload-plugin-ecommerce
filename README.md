...
- **Concurrent payment initiations for one cart are serialized**: the Stripe adapter's `initiatePayment` looks for the cart's pending transaction and creates one when there is none. Two requests arriving together both found none, so a single purchase produced two transactions, two PaymentIntents and two Stripe customers. The Stripe customer lookup, pending-transaction lookup, PaymentIntent reuse or creation and transaction write now run under `withCartLock(req, cartID, fn, options?)` (exported from the package root): a Postgres transaction-scoped advisory lock keyed on the cart's id in string form, so the second request runs only after the first has committed. A waiter never blocks inside Postgres: it polls `pg_try_advisory_xact_lock` and rolls its transaction back between attempts, so waiting holds no pooled connection (a blocking wait lets waiters fill the pool while the holder needs a connection of its own, and every database call in the application hangs), and it gives up with an error after `timeoutMs` (30 s). `resolveConnectedAccount` runs before the lock for the same reason. The upstream-style fallback that caught an `id` uniqueness error on create was removed: transaction ids are generated by the database, so that error never occurred. On non-Postgres adapters the lock is a no-op.

- **An existing PaymentIntent is reused only for the same payment, and never replaced once money moves**: `classifyExistingPaymentIntent` (exported from `payments/stripe`) decides. A payable intent (`requires_payment_method` / `requires_confirmation` / `requires_action`) is reused only when its amount, currency, customer, Connect destination and adapter-owned metadata (`cartID`, `cartItemsSnapshot`, `shippingAddress`, `connectedAccountId`) all match the payment being initiated; otherwise it is cancelled and replaced. Previously any payable intent was reused, so a cart cleared and refilled with a dearer item was charged the old amount, and `confirmOrder` built the order for the old items from the stale metadata. An intent that is `succeeded`, `processing` or `requires_capture` makes `initiatePayment` throw: replacing it would overwrite the transaction's `paymentIntentID` and orphan a payment that can then never become an order, and Stripe refuses to cancel a `processing` intent anyway.
