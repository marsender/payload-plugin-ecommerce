import type { FilterOptions, FilterOptionsProps, PayloadRequest, Where } from 'payload'
...
const readTenantCookie = (req: PayloadRequest): null | number | string => {
...
const readUserTenantIDs = (
  ...
  const memberships = (req.user as Record<string, unknown> | null)?.[arrayFieldName]
  ...
  return memberships
    .map((membership) =>
      toID((membership as Record<string, unknown> | null)?.[arrayTenantFieldName]),
    )
 ...
 */
export const resolveFilterTenantIDs = (
  args: Pick<FilterOptionsProps, 'data' | 'req'>,
  multiTenant: MultiTenantConfig,
  tenantFieldName = 'tenant',
): (number | string)[] | null => {
  const { data, req } = args

  const fromDoc = toID((data as Record<string, unknown> | undefined)?.[tenantFieldName])
  if (fromDoc !== null) {
    return [fromDoc]
  }
 ...
 */
export const tenantScopedFilterOptions = (
  multiTenant?: MultiTenantConfig,
  tenantFieldName = 'tenant',
  ...
  return (args) => {
    ...
    return { [tenantFieldName]: { in: tenantIDs } } as Where
  }
}
...
export const customerTenantFilterOptions = (
  ...
  return (args) => {
    ...
    if (multiTenant.userCanQueryCustomerTenants?.(args.req.user) === false) {
      ...
      return (userID === null ? { id: { equals: null } } : { id: { equals: userID } }) as Where
    }
    ...
    return userID === null ? inTenant : ({ or: [inTenant, { id: { equals: userID } }] } as Where)
  }
}
