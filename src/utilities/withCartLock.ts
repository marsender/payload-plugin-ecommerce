...
type PostgresLikeAdapter = {
	execute?: (args: { db?: unknown; raw: string }) => Promise<unknown>
	name?: string
	sessions?: Record<number | string, { db: unknown } | undefined>
}
 ...
 * same cart through.
 */
const toLockKey = (id: DefaultDocumentIDType): number => {
	let hash = 0x811c9dc5
 ...
 * statement).
 */
export const withCartLock = async <T>(req: PayloadRequest, cartID: DefaultDocumentIDType, fn: () => Promise<T>, options: CartLockOptions = {}): Promise<T> => {
	const db = req.payload.db as unknown as PostgresLikeAdapter
	...
	for (;;) {
		...
		try {
			...
			const row = (result as { rows?: { locked?: unknown }[] } | undefined)?.rows?.[0]
