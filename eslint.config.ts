...
/**
 * Payload local-API calls inside hooks, endpoints and payment adapters MUST share the request's
 * DB transaction by passing `req`. Without it the call takes a fresh pooled connection in its own
 * transaction: it cannot see the in-flight writes of the enclosing chain (FK violations, stale
 * reads), it commits or rolls back independently of the request (a paid Stripe intent with a
 * half-written order), and it blocks forever on any row the request transaction already locked.
 *
