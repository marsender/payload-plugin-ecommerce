 ...
 */ const describesExistingCart = async (response)=>{
    const body = await response.json().catch(()=>null);
    return typeof body === 'object' && body !== null && 'cart' in body && typeof body.cart === 'object' && body.cart !== null;
};
export const EcommerceProvider = ({ addressesSlug = 'addresses', api, cartsSlug = 'carts', children, currenciesConfig = {
   ...
   */ const postCartOperation = useCallback(async (targetCartID, operation, body)=>{
        ...
        try {
            setCart(await getCart(targetCartID, {
                secret: cartSecret
            }));
        } catch (error) {
            if (debug) {
    ...
    const addItem = useCallback(async (item, quantity = 1)=>{
        setIsLoading(true);
        try {
            if (cartID && await postCartOperation(cartID, 'add-item', {
                item,
                quantity
            }) === 'done') {
                return;
    ...
    const removeItem = useCallback(async (targetID)=>{
        ...
        setIsLoading(true);
        try {
            await postCartOperation(cartID, 'remove-item', {
                itemID: targetID
            });
        } finally{
            setIsLoading(false);
    ...
    const incrementItem = useCallback(async (targetID)=>{
        ...
        setIsLoading(true);
        try {
            await postCartOperation(cartID, 'update-item', {
                itemID: targetID,
                quantity: {
                    $inc: 1
                }
